...
double xlpos[MAXN], xls[MAXN], xle[MAXN];
double dy[MAXN];
int dat[MAXN*4];
double gans = 0.0;
...
double seg_query(int nid, int l, int r){
  if(l < r){
    if(dat[nid]){
      return dy[r] - dy[l-1];
    }
...
int main(){
  ...
  while(true){
    ...
    if(!n) break;
    for(int i=0; i<n; i++){
      scanf("%lf%lf%lf%lf", &tfx, &tfy, &ttx, &tty);
      xlid[gn] = gn;
      xlpos[gn] = tfx;
      xle[gn] = tty;
      xls[gn] = tfy;
    ...
    for(int ti=0; ti<gn; ti++){
      ...
      if(xlpos[i] != lastx){
        ...
        gans += seg_query(1, 1, yn) * (xlpos[i] - lastx);
        lastx = xlpos[i];
      }
